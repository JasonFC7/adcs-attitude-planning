...
def ECEFtoLLA(x, y, z):
    ...
    for i in range(100):                # maximum 100 iterations if calculations goes on forever
        new_beta = math.atan((1 - FLATTENING) * math.tan(lat))
        new_lat = math.atan(num(new_beta) / denom(new_beta))
        
        if new_lat == lat:              # checking if consecutive lat values are same
            beta = new_beta             
            lat = new_lat
